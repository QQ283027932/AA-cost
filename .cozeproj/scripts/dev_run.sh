...
PREVIEW_DIR="${COZE_PREVIEW_DIR:-$ROOT_DIR/logs}"
  ...
  if is_port_free "$port_val"; then
    ...
    if [ -z "$choice" ] || [ "$choice" = "y" ] || [ "$choice" = "Y" ]; then
      if command -v lsof &> /dev/null; then
        local pids
        pids=$(lsof -t -i tcp:"$port_val" -sTCP:LISTEN 2>/null || true)
        if [ -n "$pids" ]; then
          echo "正在关闭进程：$pids"
...
start_expo() {
  ...
  if [ "$offline" = "1" ]; then
    ( EXPO_OFFLINE=1 EXPO_NO_DEPENDENCY_VALIDATION=1 \
...
echo "Expo 服务已启动，进程 ID：${EXPO_PID:-unknown}"

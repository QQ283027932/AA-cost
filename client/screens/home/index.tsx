...
export default function HomePage() {
  ...
  const handleAddParticipant = async () => {
    ...
    try {
      await createParticipant(selectedActivityId, participantName.trim());
    ...
    Alert.alert(
      ...
      [
        { text: '取消', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              await deleteParticipant(activityId, participantId);
              fetchParticipants(activityId);
              fetchActivities();
            } catch (error) {
  ...
  const handleToggleParticipantStatus = async (activityId: string, participantId: string, currentStatus: string | null) => {
    ...
    try {
      await updateParticipant(activityId, participantId, {
                        ...
                        >
                          ...
                          <View style={[styles.participantDetails, { borderTopColor: theme.borderLight }]}>
                            <View style={styles.participantDetailItem}>
                              ...
                              <Text style={styles.participantDetailValue}>¥{participant.shareTotal || 0}</Text>
                            </View>
                            <View style={styles.participantDetailItem}>
                              <Text style={styles.participantDetailLabel}>支付总额</Text>
                              <Text style={styles.participantDetailValue}>¥{participant.paidTotal || 0}</Text>
                            </View>
                          </View>
                        </ThemedView>
                      ))
